...
const Playlist = () => {
    ...
    useEffect(() => {
        if (profile?.name) {
            fetchAllPlaylists(dispatch, profile.name); // pass profile name
        }
        console.log('Fetching playlists for:', profile.name);
    }, [dispatch, profile.name]);

    const handleSubmit = async (e) => {
        e.preventDefault();
        if (label.trim() !== '') {
            await createPlaylist(dispatch, label, profile.name);
